...
| 4 | 129 | **Offline ticket access** | Requested from the internal test (12 Aug 2026): a fan should be able to open a **purchased ticket (QR + show details)** with no connection — venues have bad signal, and a ticket you can't load is a ticket you can't use. The QR encodes just a ticket UUID string (e.g. `73202d65-…`), so the fix is to **cache each ticket's data locally** (Cache API / IndexedDB / localStorage) and render the QR offline from the cached UUID + show fields. Extends the #110 service worker, which **deliberately never caches authed HTML** — so this needs a *targeted* ticket cache (only the user's own tickets, refreshed when online), not blanket caching of authed pages. Benefits web PWA and the native shell equally. | Nothing — client-side cache + offline render. | M |
...
| 1 | 131 | **Native iOS: Stripe checkout leaves the app and returns to the browser** | Reported from the internal test (12 Aug 2026). In the native shell, tapping to pay opened Stripe Checkout **outside the WKWebView** (system browser), and the post-payment return URL then **loaded in Safari instead of the app** — the fan ended up in Safari, not on their ticket. Not a payments bug (Stripe itself works): Capacitor punts any off-origin navigation to the system browser, so `checkout.stripe.com` opened externally and the return URL (on our own domain) stayed there. **Code landed 13 Aug 2026, awaiting rebuild + device test:** `server.allowNavigation: ["*.stripe.com"]` in `capacitor.config.ts` (synced into the iOS project) lets the **main webview** navigate Stripe in-app, so Checkout + Connect payout onboarding run inline and the return URL lands back in the app on its own — no server/checkout-code change, no deep link needed. Test-mode 3-D Secure stays on `*.stripe.com`; *(live-mode 3DS can bounce to arbitrary bank domains — revisit before the payments go-live, #98/#99.)* **Native config, so it needs a fresh TestFlight build to test.** | Me (done — allowNavigation), you (rebuild + device retest: buy a test-mode ticket, confirm you land on the ticket in-app, and that cancel returns cleanly). | M |
| — | 131b | **Native iOS: external ticket links open in an in-app browser (with an escape hatch)** | Split from #131. External-ticketing shows send fans to third-party sites (Entradium, DICE…), which we can't allowlist — so instead of punting to Safari, they now open an **in-app browser sheet** (`Browser.open` → SFSafariViewController) that keeps the fan in MadGigz and carries iOS's built-in **"open in Safari"** button. **Shipped in code 13 Aug 2026:** new `openExternal()` in `src/lib/native.ts` (web = new tab, unchanged; native = in-app sheet), wired into `TicketModal` and `PublicEventActions`. **Web-bundle change — the `@capacitor/browser` plugin is already in the binary (OAuth uses it), so this reaches the installed app on next launch WITHOUT a rebuild, once prod is deployed.** | Me (done), you (device retest after prod deploy). | S |
| 2 | 132 | **"Check your email" screen needs a way back to sign in (+ verification didn't complete)** | Reported from the internal test (12 Aug 2026): after signup the account **read as unverified in the app** and the "check your email" screen was a **dead end**. Diagnosed: the confirmation link in the inbox opens in **Safari**, not the app's WKWebView — `/auth/confirm` verifies the account and sets the session cookie **in Safari's cookie jar**, which the app's separate webview never sees, so the fan returns to the app still logged out. The account *is* verified; the app just didn't receive the session. **Fix shipped in code 13 Aug 2026 (web bundle, no rebuild):** the verify-email screen now has an **"Already confirmed? Sign in"** link → `/signin` (carrying `email` + `next`), and `/signin` prefills the field from `?email=`. So the fan confirms in Safari, comes back, taps Sign in, and logs in with the email+password they just set — in-webview, where the session lands. Verified on device 13 Aug 2026: the sign-in path works and the email is prefilled — so the dead end is gone. **Residual friction (accepted for beta):** the confirm link still opens in **Safari** (iOS hands `https://` links to the browser unless the app claims them), so confirmation lands in Safari's cookie jar and the fan must still **re-enter their password** on returning to the app. Two ways to remove it completely, both **deferred** (bigger than a tweak): **(a) Universal Links** — Associated Domains entitlement + host `apple-app-site-association` at `madgigz.aurasonic.es/.well-known/`, so the email link opens the **app**, confirmation runs in-webview, and the fan goes straight to the feed (no Safari, no password); the proper pre-public-launch fix. **(b) Email OTP code** — put `{{ .Token }}` (6-digit code) in the Supabase confirmation template and add a code input to the verify-email screen; `verifyOtp` runs in-app → session lands in-app, no browser at all (medium effort, also benefits web). Relates to #78. | Me (done — sign-in path; a/b deferred), device-verified. | S |
