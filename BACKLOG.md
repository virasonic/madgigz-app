...
| 3 | 113 | **Move Log Out + Delete account into Settings** | On both fan and artist profiles, **Log Out** and **Delete account** currently sit in the profile page body (`ProfileClient.tsx` ~L464/L474). Move them into the Settings sheet (`SettingsSheet`, same file) where the other account controls live — Log Out as a normal row, Delete account kept visually quiet/destructive at the bottom (it already is). Small, tidies the profile. | Nothing. | S |
| 3 | 114 | **Show posters in the artist's "Your Shows" list** | The artist profile's "Your Shows" list (`ProfileClient.tsx` ~L376) is text rows; add each show's **poster thumbnail** so an artist scanning their shows recognises them at a glance instead of reading titles. The image URL is already on the event. | Nothing. | S |
