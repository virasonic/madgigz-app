...
| 3 | 119 | **Terms & conditions (+ the legal pages)** | The user-facing legal set the app needs before real money changes hands: **Terms of Service** (the fan↔MadGigz and artist↔MadGigz agreements — MadGigz as the marketplace/intermediary, not the promoter; ticketing terms, refunds/cancellations, the per-ticket fee, artist payout terms), plus the **Privacy Policy** and **cookie notice** already flagged as store + GDPR requirements (see #110, #112). Needs: the actual copy (Vir now **has more clarity** on what the terms should say — to detail), acceptance capture at signup (a checkbox + a stored `accepted_terms_at` / version, so re-consent works when terms change), linked static pages the app and the marketing site both point to, and **both languages** through the i18n catalog (legal copy is user-facing). A public privacy + account-deletion page is also a hard Google Play requirement (#110), so this unblocks that too. **This is a go-live prerequisite, not polish** — real payments (#95) shouldn't flip without it. | Vir to supply/approve the actual legal wording (a gestor/lawyer pass is wise); everything else is straightforward once the copy exists. | M |
| 7 | 120 | **Desktop: announcements panel beside the feed, triggered from the sidebar** | Web-only polish on the #105 foundation. Today the "From MadGigz" announcements open as a **bottom-sheet modal that slides up over the centred For You card**, which feels cramped and awkward on a wide screen (the phone-first affordance stranded on a monitor). Two changes, desktop only: **(1)** move the megaphone/announcements trigger **into the `SideNav`** (a first-class rail item alongside Feed/Explore/… ), off the feed header where it competes with the pane toggle; **(2)** at `lg`+, render the announcements as a **column beside the feed** (the empty left/right canvas the centred reel already leaves), not an overlay — so feed and announcements read side by side. **Mobile is untouched** — it keeps the current header trigger + bottom sheet, which is right for a phone. Reuses the existing announcements data/`AnnouncementCard`; this is layout + trigger placement, no new backend. | Nothing — pure frontend, rides on #105's `SideNav`/desktop shell. | S–M |
