...
| 2 | 128 | **Native iOS: session dropped on every app close (must re-login)** | Found in the first on-device internal test (12 Aug 2026). Closing and reopening the iOS app forces a fresh sign-in every time — the WKWebView isn't persisting the Supabase auth session across app launches. The web app is unaffected (browsers persist cookies), so this is native-shell-specific: either the WKWebView cookie store isn't persisting the `@supabase/ssr` session cookies across process restarts, or those cookies are being written as session-cookies (no expiry) rather than persistent ones. Investigate: WKWebView `WKWebsiteDataStore` persistence under Capacitor's `server.url` remote load, the `CapacitorCookies` config, and whether the Supabase session/refresh cookies carry a `maxAge`/`Expires`. **High priority — makes the native app frustrating to use.** Likely a Capacitor cookie-persistence config + confirming persistent cookie expiry; possibly a small native tweak, no web change. | Nothing — investigation + config. | S–M |
...
| 6 | 130 | **Bottom sheets should swipe-down to dismiss** | From the internal test (12 Aug 2026): the ticket sheet (and other bottom sheets) show a **drag handle** but can't actually be dragged down to close — the affordance is misleading. Add **swipe-down-to-dismiss** to the shared bottom-sheet/`TicketModal` component (drag past a threshold → close; snap back otherwise), keeping the existing tap-outside/✕ close. Applies to **web + native** (same component), and matters more on iOS where swipe-to-dismiss is the expected gesture. | Nothing — gesture handling on the sheet component. | S |
