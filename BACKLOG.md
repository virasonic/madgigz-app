...
| 3 | 122 | **Google OAuth: publish to Production + brand the sign-in domain** | Two Google-sign-in tasks that surfaced 12 Aug 2026 while Vir was tidying the consent screen — **both live in Google Cloud / Supabase dashboards, no app code.** **(a) Publish to Production (launch blocker, easy):** the OAuth app is in **Testing**, so only explicitly-added test users can sign in with Google. MadGigz only requests non-sensitive scopes (name, email, profile pic), so it can be published to Production with no Google verification review and no "unverified app" warning — one **Publish app** click in the OAuth consent screen. Must be done before real users hit Google sign-in at launch. **(b) Brand the redirect host (cosmetic):** the sign-in + consent screens read *"to continue to `rxtiagsypwvuyyihbhal.supabase.co`"* because Google shows the true redirect host, and it only shows a *name* for a domain you've **verified ownership of** — which you can't for `supabase.co`. The App-name field can't override it. The only fix is the **Supabase Custom Domains add-on (~$10/mo per project)**: point auth at e.g. `auth.aurasonic.es`, add the DNS records, verify that domain in Google, then update the Google OAuth client's Authorized redirect URI + Supabase Site/redirect URLs and re-test. App name is already set to MadGigz; privacy/terms already link the real `aurasonic.es` pages (#119). Bundle with go-live (#95). | (a) nothing — one click, do before launch. (b) the paid Supabase add-on + DNS on `aurasonic.es`; pairs with #95. | S (publish) · M (custom domain) |
