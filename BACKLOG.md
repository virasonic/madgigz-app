...
| 2 | 107 | **Back buttons where needed** | Audit the full-screen pushed routes that sit outside the bottom-nav shell and currently rely on the browser/OS back gesture alone — Add Show, Scan tickets, Edit profile, the payout/settings sheets, the ticket QR screen, the admin sub-pages. Add a consistent in-app back affordance (a shared header-back component) wherever a screen is a dead-end without one, so it doesn't depend on a gesture a home-screen PWA may not show. Needs a quick pass to list exactly which screens qualify. | Nothing. | S |
