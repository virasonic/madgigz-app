...
| 3 | 122 | **Google OAuth: publish to Production + brand the sign-in domain** | **(b) Brand the redirect host — SHIPPED to prod 12 Aug 2026.** The sign-in screens no longer read *"to continue to `rxtiagsypwvuyyihbhal.supabase.co`"* — the **prod** Supabase project now has a **Custom Domain** (`auth.aurasonic.es`, ~$10/mo add-on): CNAME on GoDaddy → Supabase Activate → Google OAuth client redirect URI added (`https://auth.aurasonic.es/auth/v1/callback`, old one kept) → `NEXT_PUBLIC_SUPABASE_URL` flipped to the custom domain (**Production scope only** — Preview stays on staging's Supabase, #108) → prod redeployed (`NEXT_PUBLIC_*` is build-time inlined, so the client bundle only switches on rebuild; server routes flip at runtime). Verified: `/api/health` `supabaseHost: auth.aurasonic.es`, authorize endpoint returns `redirect_uri=…auth.aurasonic.es/…`, app + REST 200 over the custom domain. Media-URL→path converters made host-agnostic (`storage.ts`, `account-deletion.ts`) so pre-cutover objects still resolve. Runbook in `docs/go-live.md`. Staging stays on `supabase.co`. **(a) Publish to Production — STILL OPEN, launch blocker:** the OAuth app is in **Testing**, so only added test users can sign in with Google. Non-sensitive scopes only → one **Publish app** click, no verification review, no warning. Must be done before real users hit Google sign-in. | **(a) one click before launch** (OAuth consent screen → Publish app). (b) done. | S (publish) · ✅ (domain) |
| 6 | 123 | **Show the artist's profile photo on feed reels** | The For You reel card (`ContentReelCard`) shows a generic music-note avatar (`NoteIcon`) next to the artist's name — it should show their actual **profile picture** (`profiles.artist_photo_url`), falling back to the note icon only when they haven't set one. `ContentPost` doesn't currently carry the photo (`content_posts` stores `artist_name` but not the URL), so the work is: **(1)** surface `artist_photo_url` when loading posts — join `content_posts.artist_id → profiles.artist_photo_url` in `fetchContentPosts` and add the field to the `ContentPost` type/mapper; **(2)** render it in the card avatar (`<Image fill>` in the existing circle, `NoteIcon` fallback). Small. The photo is already public-API-readable (the artist profile query selects it), so no new grant. **Note:** MadGigz's own **announcements** keep the MadGigz logo (`AnnouncementCard`) — this is artist reels only. Same treatment could extend to the mobile/desktop avatar wherever a reel shows an artist. | Nothing — data + one component. | S |
