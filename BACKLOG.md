...
| 4 | 133 | **Notifications/announcements sheet feels a little slow on device** | Noted in the 13 Aug 2026 device test ("Notes, a little slow, but it works"). The notifications bell / announcements sheet has a perceptible lag opening on the native shell. Not broken — a polish item. Likely a data-fetch-on-open (no prefetch/cache) or a heavy render; profile before optimising. Check whether it fetches every open vs. caches, and whether the realtime subscription (`src/lib/realtime.ts`) is doing work on mount. **Vir: tackle in the next push (14 Aug 2026).** | Me — profile + trim. | S |
...
| 3 | 135 | **Feed reels slow to start playing (cold first-play latency)** | Flagged 13 Aug 2026 (main admin account: reels took a long time to start after login). Root of it in `ContentReelCard.tsx`: the `<video>` has a `poster` but **no `preload`**, and nothing **prefetches the next reel** — each video only begins loading when it scrolls into view (IntersectionObserver → `video.play()`), so the first reel has to fetch the whole file **cold from Supabase Storage** (no CDN warmth, no transcode) before it can play. Levers: add `preload` and/or warm the next 1–2 videos' first bytes; ensure the poster paints instantly so there's never a blank frame; longer term, transcode/serve adaptively (ties to #100 video editor and #101 storage/bandwidth watch). Profile first — measure time-to-first-frame on device before/after. Benefits web + native. | Me — profile + prefetch/preload. | M |
| 5 | 136 | **Pull-to-refresh on the feed (pull down on the top reel to refresh)** | Requested 13 Aug 2026: in the app, pulling down while on the **top** video should refresh the feed (fetch new reels/shows), the standard native gesture. Implementation notes: the feed is a vertical scroll-snap column — trigger only when the container is at `scrollTop === 0` and the gesture is a downward pull, and don't let it fight the existing per-reel interactions or the swipe-to-dismiss sheets (#130). Show a refresh affordance (spinner/pull indicator) and re-run the feed fetch (`router.refresh()` or a client refetch of `fetchContentPosts`/shows). Native shell + web PWA both benefit; on plain desktop web it's a no-op. Watch iOS WKWebView rubber-band/overscroll so the browser's own bounce doesn't swallow the gesture. | Me — gesture + refetch. | M |
