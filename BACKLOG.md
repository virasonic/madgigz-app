...
| 8 | 106 | **Pin the Explore search + genre filters** | On Explore the search bar and the genre chips scroll away with the grid; they should stay put while only the events scroll beneath. In `ExploreClient` (`src/app/(app)/explore/ExploreClient.tsx`), lift the title / search input / genre-chip row into a `sticky top-0` bar carrying the page background (and a `z-index` above the cards), and let the grid be the only thing that scrolls. Self-contained UI change. | Nothing. | S |
