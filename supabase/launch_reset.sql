...
--
-- WHAT IT DOES
--   Deletes fake/test events. Because of the foreign keys (schema.sql), deleting
--   an event CASCADES to everything hanging off it:
--     tickets, content_posts, event_artists, event_genres, saved_events.
...
--   reads €0 on its own. Deleting the events IS the reset.
--
...
-- current shows are fake). Uncomment the line and run it.
--
...
-- keep them. Paste their ids (from STEP 1) into the list, then uncomment + run.
-- Any test tickets on the KEPT shows would still count toward revenue, so keep
-- only shows that genuinely have no test purchases.
--
-- delete from public.events
-- where id not in (
