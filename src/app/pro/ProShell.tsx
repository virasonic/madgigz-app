...
import { ProAccountType } from "@/lib/pro";
import { useT } from "@/lib/i18n/LocaleProvider";
import type { Locale } from "@/lib/i18n/config";
import SetPanelLanguage from "./SetPanelLanguage";

// The panel IS translated, unlike /admin. Promoters and venues are third
// parties rather than MadGigz's own back office (Vir, 20 Sept 2026), so it
// follows the language their account was set up in - see the LocaleProvider in
// layout.tsx and requirePro()'s `t` for the server half.
const NAV_ITEMS = [
  { href: "/pro", key: "pro.navDashboard" },
  { href: "/pro/events", key: "pro.navEvents" },
  { href: "/pro/payouts", key: "pro.navPayouts" },
];
  ...
  return (
    <div className="flex items-center gap-2">
      <Image
        ...
        priority
      />
      <span className="font-heading text-[10px] uppercase tracking-widest text-muted">{suffix}</span>
    </div>
  );
}
...
export default function ProShell({
  ...
  const footer = (
    <div className="mt-auto flex flex-col gap-3 pt-6 text-xs text-muted">
      <div className="flex flex-col gap-1">
        <span className="text-foreground">{displayName}</span>
        <span>{t(type === "venue" ? "pro.accountVenue" : "pro.accountPromoter")}</span>
      </div>
      <SetPanelLanguage current={locale} />
    ...
    <div className="flex h-dvh overflow-hidden bg-background text-foreground">
      ...
      <aside className="hidden w-60 shrink-0 flex-col overflow-y-auto border-r border-muted/15 px-3 py-5 lg:flex">
        <Link href="/feed" className="mb-6 px-2" aria-label="MadGigz Pro">
          <Wordmark suffix={t("pro.panelSuffix")} />
        </Link>
      ...
      {drawerOpen && (
        ...
        >
          ...
          <aside className="pt-safe-page relative flex w-64 max-w-[80%] flex-col overflow-y-auto border-r border-muted/15 bg-background px-3 pb-6">
            ...
            <NavLinks pathname={pathname} t={t} onNavigate={() => setDrawerOpen(false)} />
            {footer}
          </aside>
        </div>
      )}
