...
export async function generateMetadata({
  ...
  if (!event || !event.active) return { title: "Event not found - MadGigz" };
  ...
  return (
    <div className="mx-auto flex min-h-screen w-full max-w-md flex-col bg-background">
      <header className="pt-safe-page flex items-center justify-between px-5 pb-4">
        <Link href="/" aria-label="MadGigz home">
          <Image
      ...
      <div className="px-5 pb-12">
        ...
        {event.cancelled && (
          <p className="mt-4 rounded-xl bg-primary/10 px-4 py-3 font-heading text-sm text-primary">
            This show has been cancelled. Any tickets bought through MadGigz have
            been refunded.
          </p>
        )}
          ...
          <>
            ...
            <ol className="flex flex-col gap-1 text-sm text-foreground/90">
              {event.lineup.map((act, index) => (
                <li key={act}>
                  {index + 1}. {act}
                  {index === 0 && <span className="ml-2 text-xs text-accent">Headliner</span>}
                </li>
              ))}
            </ol>
          </>
